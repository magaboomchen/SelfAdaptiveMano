...
def addSFC():
    ...
    with grpc.insecure_channel("192.168.122.134:10514") as channel:
        ...
        argument = Any()
        argument.Pack( 
                port_msg_pb2.PMDPortArg(
                    loopback=True,
                    vdev="net_vhost0,iface=/tmp/vsock",
                    vlan_offload_rx_strip=False,
                    vlan_offload_rx_filter=False,
                    vlan_offload_rx_qinq =False
                )
            )
        response = stub.CreatePort(
                bess_msg_pb2.CreatePortRequest(
                    name="vhost",driver="PMDPort",
                    num_inc_q=1,num_out_q=1, size_inc_q=0, size_out_q=0, 
                    arg=argument
                )
            )
        if response.error.code != 0:
        ...
        argument.Pack(
                port_msg_pb2.PMDPortArg(
                    loopback=True,
                    vdev="net_vhost2,iface=/tmp/vsock2",
                    vlan_offload_rx_strip=False,
                    vlan_offload_rx_filter=False,
                    vlan_offload_rx_qinq =False
                ) 
        ...
        response = stub.CreatePort(
            bess_msg_pb2.CreatePortRequest(
                name="vhost2",driver="PMDPort",num_inc_q=1,num_out_q=1, size_inc_q=0, size_out_q=0, arg=argument
            )
        )
        if response.error.code != 0:
            logging.error( str(response.error) )
        ...
        ## create
        ## vhostpmd_in::QueueInc(port="vhost", qid=0)
        argument = Any()
        argument.Pack( module_msg_pb2.QueueIncArg(port="vhost", qid=0))
        response = stub.CreateModule(
            bess_msg_pb2.CreateModuleRequest(
                name="vhostpmd_in",mclass="QueueInc",arg=argument
            )
        )
        if response.error.code != 0:
            logging.error( str(response.error) )
        ...
        ## create
        ## vhostpmd_out::QueueOut(port="vhost2", qid=0)
        argument = Any()
        argument.Pack( module_msg_pb2.QueueOutArg(port="vhost2", qid=0))
        response = stub.CreateModule(
            bess_msg_pb2.CreateModuleRequest(
                name="vhostpmd_out",mclass="QueueOut",arg=argument
            )
        )
        if response.error.code != 0:
            logging.error( str(response.error) )
...
def delSFC():
    ...
    with grpc.insecure_channel("192.168.122.134:10514") as channel:
        ...
        argument = Any()
        argument.Pack(
            module_msg_pb2.ExactMatchCommandDeleteArg(
                fields=[{"value_int":17}, {"value_bin":aton("192.168.122.111")}, {"value_int":htonl(999)}]
            )
        )
        ...
        # ExactMatch
        # ## em::ExactMatch(fields=[{"offset":23, "num_bytes":1},{"offset":30, "num_bytes":4},{"offset":45, "num_bytes":4}])
        # response = stub.DestroyModule(bess_msg_pb2.DestroyModuleRequest(name="em"))
        # if response.error.code != 0:
        #     logging.error( str(response.error) )
        # else:
        #     logging.info("DestroyModuleResponse -- name: ")

        ## vhostpmd_in::QueueInc(port="vhost", qid=0)
        response = stub.DestroyModule(bess_msg_pb2.DestroyModuleRequest(name="vhostpmd_in"))
        if response.error.code != 0:
            logging.error( str(response.error) )
        else:
            logging.info("DestroyModuleResponse -- name: ")
        ...
        ## vhostpmd_out::QueueOut(port="vhost2", qid=0)
        response = stub.DestroyModule(bess_msg_pb2.DestroyModuleRequest(name="vhostpmd_out"))
        if response.error.code != 0:
            logging.error( str(response.error) )
        else:
            logging.info("DestroyModuleResponse -- name: " )
        ...
        ## PMDPort
        # vhostpmd = PMDPort(name="vhost", vdev="net_vhost0,iface=/tmp/vsock")
        response = stub.DestroyPort(bess_msg_pb2.DestroyPortRequest(name="vhost"))
        if response.error.code != 0:
            logging.error( str(response.error) )
        else:
            logging.info("DestroyPortResponse -- name: " )

        # vhostpmd2 = PMDPort(name="vhost2", vdev="net_vhost2,iface=/tmp/vsock2")
        response = stub.DestroyPort(bess_msg_pb2.DestroyPortRequest(name="vhost2"))
        if response.error.code != 0:
            logging.error( str(response.error) )
        else:
            logging.info("DestroyPortResponse -- name: " )
