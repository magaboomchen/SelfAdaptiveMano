...
class NorthSouthRouting(BaseApp):
    ...
    def _packet_in_handler(self, ev):
        ...
        if dpid == self._defaultDCNGateway and msg.table_id == L2_TABLE and eth.ethertype == ether_types.ETH_TYPE_ARP:
            ...
            if (in_port == DEFAULT_DCN_GATEWAY_OUTBOUND_PORT_NUMBER and 
                arpHeader.opcode == arp.ARP_REPLY and 
                self._defaultDCNGateway != None and 
                arpHeader.src_ip == self._switchConfs[self._defaultDCNGateway].dcnGatewayPeerIP):
        ...
        if dpid == self._defaultDCNGateway and eth.ethertype == ether_types.ETH_TYPE_ARP:
            arpHeader = pkt.get_protocol(arp.arp)
            if (arpHeader.opcode == arp.ARP_REQUEST and
                arpHeader.dst_ip == self._switchConfs[self._defaultDCNGateway].dcnGatewaySelfIP):
