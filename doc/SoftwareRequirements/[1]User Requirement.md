...
3. 允许租户通过Web页面的方式申请、查看和删除SFC；

4. 允许租户通过Web页面的方式上传VNF配置规则；
...
7. 通过NFV管理和编排系统的Web页面实时查看每条SFC的服务等级指标（Service Level Indicator，SLI），包括SFC的时延、丢包率、吞吐量、VNF/server的IP地址，VNF instance的数量和VNF的资源使用情况；可以实时获知每条SFC是否出现了故障，故障保护是否成功，故障恢复是否成功；可以实时获知每条SFC的资源利用率，是否需要垂直扩展和水平扩展；（流量检测模块）
...
1. 租户1的网站部署在该公有云上，他希望到达其网站的流量经过特定的SFC；[\[1\]][1] （该场景与移动核心网的SFC服务类似，同属南北向流量）
...
1. 单节点故障保护率100%；（故障保护：发生硬件/软件故障后，利用预留的资源恢复SFC）

2. 水平扩展时间低于10s；（水平扩展：在第二台server上启动新的VNF实例，占用额外的资源来处理更多的流量）

3. 垂直扩展时间低于1s；（垂直扩展：VNF在同一台server上占用更多的资源来处理更多的流量）

4. 人为蓄意扰动抑制率>95%；（用户产生特殊的流量对编排管理系统发动攻击，比如频繁欺骗MANO进行水平扩展，消耗MANO控制平面的计算资源）

5. DCN网关支持百万级的流表分类；（需要对流量进行分类，识别出每个流所属的SFC，并实现SFC）

6. 故障恢复时间低于10s；（故障恢复：发生硬件/软件故障后，临时分配资源恢复SFC）

租户的需求：

1. VNF种类要求：3-5种，包括FW, IDS, VPN(IPsec), Monitor, Load Balancer, NAT

2. SFC顺序要求：比如FW -> Load Balancer

3. 可用度要求：包括>99.5%、>99.9%、>99.99%、>99.999%、99.9999%

4. 丢包率要求：比如99%分位数小于5%，并且90%分位数小于2.5%，并且75%分位数小于1%
